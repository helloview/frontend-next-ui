...
type WorldviewProvider = "gemini" | "ollama";
type OllamaThinkMode = "none" | "boolean" | "string";
type GeminiThinkingLevel = "minimal" | "low" | "medium" | "high";

type GenerateWorldviewRequestBody = {
...
function normalizeProvider(value: unknown): WorldviewProvider {
...
function composeWorldviewPrompt(script: string, instruction: string): string {
  return [
    "你是影视创作团队的设定总监，请根据剧本文稿整理人物与环境设定。",
    "你必须严格输出 JSON 对象，不要输出 Markdown，不要输出解释。",
    "输出 Schema（字段名必须保持中文）：",
    "{",
    '  "人物": [',
    "    {",
    ...
    "}",
    "规则：",
    "1. name 必填，title 可与 name 相同。",
    "2. 视觉信息必须是对象，键值全部为字符串。",
    "3. 没有对应内容时返回空数组，不要编造无关字段。",
    "4. 严禁输出代码块包裹符号，例如 ```json。",
    "",
    "【剧本文稿】",
    script,
    "",
...
function extractErrorMessage(payload: unknown): string {
  ...
  return getString(record.details);
}
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as GenerateWorldviewRequestBody | null;
  const provider = normalizeProvider(body?.provider);
  const script = getString(body?.script);
  const instruction = getString(body?.instruction) || "请按剧情阶段完整提取人物与环境信息。";
  ...
  const prompt = composeWorldviewPrompt(script, instruction);

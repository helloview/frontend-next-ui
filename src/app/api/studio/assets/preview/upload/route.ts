...
type AssetPreviewType = "characters" | "environments";
...
function extractErrorMessage(payload: unknown): string {
  ...
  return getString(record.error) || getString(record.message) || getString(record.detail) || "";
}
...
function isAssetPreviewType(value: string): value is AssetPreviewType {
  return value === "characters" || value === "environments";
}
...
export async function POST(request: Request) {
  ...
  const assetId = getString(form.get("assetId"));
  const assetTypeRaw = getString(form.get("assetType"));
  if (!assetId) {
    return NextResponse.json({ error: "assetId is required" }, { status: 400 });
  }
  if (!isAssetPreviewType(assetTypeRaw)) {
    return NextResponse.json({ error: "assetType must be characters or environments" }, { status: 400 });
  }
  ...
  const fileName = sanitizeFileName(file.name || `asset-preview-${Date.now()}.png`) || `asset-preview-${Date.now()}.png`;
  const subject = session.user.id || session.user.email || "studio-user";
  const objectKey = `${assetTypeRaw}-previews/${assetId}/${Date.now()}-${randomUUID()}-${fileName}`;

  const metadata = {
    source: "library-asset-preview-upload",
    assetId,

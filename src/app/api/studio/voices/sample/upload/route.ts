...
export async function POST(request: Request) {
  ...
  const voiceID = getString(form.get("voiceId"));
  ...
  const fileName = sanitizeFileName(file.name || `voice-sample-${Date.now()}.mp3`) || `voice-sample-${Date.now()}.mp3`;
  const subject = session.user.id || session.user.email || "studio-user";
  const objectKey = `voice-samples/${voiceID || subject}/${Date.now()}-${randomUUID()}-${fileName}`;
  const metadata = {
    source: "library-voice-sample-upload",
    voiceId: voiceID,
    uploadedBy: subject,
  };
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      uploadForm.append("content_type", file.type || "audio/mpeg");
